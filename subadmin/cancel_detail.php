...
if(isset($_GET['id']) && $_GET['id']>0){
  ...
  if(isset($_GET['procecutor'])){
    $procecutor=get_safe_value($con,$_GET['procecutor']);
    mysqli_query($con,"update cases set pcheck_id='$procecutor' where id='$id'");
    redirect(SITE_PATH.'subadmin/cancel_detail.php?id='.$id);
  }
            ...
            <div class="row">
                  ...
                  <div class="card-body">
                    ...
                    <div class="container-fluid mt-5 d-flex justify-content-center w-100">
                      <div class="table-responsive w-100">
                        ...
                        <?php 
                          $prosecution_res=mysqli_query($con,"select * from prosecution where decision='canceled' and status='1' and cases_id='".$courtRow['id']."' order by id asc");
                        ?>
                        <p>
                          <?php
                          while($prosecution_row=mysqli_fetch_assoc($prosecution_res)) {
                          ?>
                            <h4><strong>Prosecutor</strong>&nbsp;<small>Statement</small></h4>
                            <?php
                    ...
                    <div class="container-fluid w-100">
                      <a href="#" class="btn btn-primary float-right mt-4 ml-2"><i class="mdi  mdi-folder-download mr-1"></i>Download</a>
                        ...
                        <?php
                        $procecutorAssignRes=mysqli_query($con,"select * from juser where role_id='1' and status=1 order by id asc");
                        ?>
                        <div>
                        <?php
                        echo "<h4>Reassign to Prosecutor :- ".getProcecutorNameById($courtRow['prosecutor_id'])."</h4>"
                        ?>
                        <select name="procecutor" id="procecutor" class="form-control w200" onclick="updateprocecutorAssign()">
                            <option value="">Assign to Prosecutor</option>
                            <?php
                            while($procecutorAssignRow=mysqli_fetch_assoc($procecutorAssignRes)){
                                echo "<option value=".$procecutorAssignRow['id'].">".$procecutorAssignRow['name']."</option>";
                            }
                            ?>
                      </select>
                    </div>
              ...
              <script>
                function updateprocecutorAssign(){
                  ...
                  if(procecutor!=''){
                    var oid="<?php echo $id?>";
                    window.location.href='<?php echo SITE_PATH?>subadmin/cancel_detail.php?id='+oid+'&procecutor='+procecutor;
                  }
                }
              </script>
